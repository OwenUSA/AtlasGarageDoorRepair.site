// scripts/palette.mjs — randomize hue, preserve structure.
//
//   node scripts/palette.mjs                 generate 5 candidates, gate, auto-select
//   node scripts/palette.mjs --seed 12345    reproduce one palette exactly
//   node scripts/palette.mjs --emit          print the winning @theme block
//   node scripts/palette.mjs --json          machine-readable
//
// What makes a palette read as designed is the lightness and chroma relationships across
// the ramp, not the hue. So: convert the Prompt 1 extracted ramp to OKLCH, hold every L
// and C exactly where it is, and re-derive the whole set from a new random primary hue.
//
// Semantic colors (form error, form success, focus ring) are EXEMPT from rotation and
// keep conventional hues. A randomly green error state is a bug.
...
// ---------------------------------------------------------------- the extracted ramp
// Measured in Prompt 1 from the reference's computed styles. These L and C values are the
// STRUCTURE that survives randomization; only H moves.
export const REFERENCE_RAMP = {
  primary:     '#132f54', // rgb(19,47,84)    navy — structural dark, 129 occurrences
  primaryDeep: '#0d2039', // derived darker step for the sticky header edge
  accent:      '#bb2026', // rgb(187,32,38)   red — CTA and banded emphasis, 84 occurrences
  accentDeep:  '#6b040b', // rgb(107,4,11)    the dark stop of the band gradient
  neutral0:    '#ffffff', // rgb(255,255,255) page surface
  neutral200:  '#e8e8e8', // hairline / subtle fill
  neutral400:  '#acacac', // rgb(172,172,172) footer + CTA band
  neutral600:  '#666666', // rgb(102,102,102) body text, 264 occurrences
  neutral900:  '#1a1a1a', // strong ink
};

/** Roles that are EXEMPT from hue rotation. Conventional hues, always. */
export const SEMANTIC = {
  error:   '#b3261e', // conventional red
  success: '#1e7a3c', // conventional green
  warning: '#8a5a00',
};
...
export function generate(seed) {
  ...
  const ref = Object.fromEntries(
    Object.entries(REFERENCE_RAMP).map(([k, hex]) => [k, rgbToOklch(hexToRgb(hex))])
  );
  ...
  // #acacac hairline is 2.27:1 on white and fails; it is kept for DECORATION only and a
  // separate strong token is introduced for anything a user has to perceive.
...
// Not the ramp in theory — these are the foreground/background combinations the shell and
// the section components actually render. Kept in step with the build.
export const PAIRS_IN_USE = [
  { name: 'body text on surface',        fg: 'neutral600', bg: 'neutral0',    min: 4.5, kind: 'text' },
  { name: 'strong ink on surface',       fg: 'neutral900', bg: 'neutral0',    min: 4.5, kind: 'text' },
  { name: 'heading on surface',          fg: 'primary',    bg: 'neutral0',    min: 4.5, kind: 'text' },
  { name: 'link on surface',             fg: 'accent',     bg: 'neutral0',    min: 4.5, kind: 'text' },
  { name: 'white on primary band',       fg: 'neutral0',   bg: 'primary',     min: 4.5, kind: 'text' },
  { name: 'white on top bar',            fg: 'neutral0',   bg: 'primaryDeep', min: 4.5, kind: 'text' },
  { name: 'white on accent band',        fg: 'neutral0',   bg: 'accent',      min: 4.5, kind: 'text' },
  { name: 'white on accent deep',        fg: 'neutral0',   bg: 'accentDeep',  min: 4.5, kind: 'text' },
  { name: 'CALL-NOW CTA label on CTA',   fg: 'neutral0',   bg: 'accent',      min: 4.5, kind: 'cta' },
  { name: 'ink on footer band',          fg: 'neutral900', bg: 'neutral400',  min: 4.5, kind: 'text' },
  { name: 'heading on footer band',      fg: 'primary',    bg: 'neutral400',  min: 4.5, kind: 'text' },
  { name: 'large heading on surface',    fg: 'primary',    bg: 'neutral0',    min: 3.0, kind: 'large' },
  { name: 'input border on surface',     fg: 'borderStrong', bg: 'neutral0',  min: 3.0, kind: 'ui' },
  { name: 'divider on footer band',      fg: 'primary',    bg: 'neutral400',  min: 3.0, kind: 'ui' },
  { name: 'secondary btn label on surf', fg: 'primary',    bg: 'neutral0',    min: 4.5, kind: 'action' },
  // Focus ring is two-layer: surface-coloured inner halo, dark outer ring. Both adjacencies
  // are gated, which is what "3:1 against both the element and its background" means for a
  // ring that sits on an offset.
  { name: 'focus ring vs its halo',      fg: 'focus',      bg: 'neutral0',    min: 3.0, kind: 'focus' },
  { name: 'focus ring vs footer band',   fg: 'focus',      bg: 'neutral400',  min: 3.0, kind: 'focus' },
  { name: 'focus ring vs surface',       fg: 'focus',      bg: 'neutral0',    min: 3.0, kind: 'focus' },
  { name: 'focus halo vs primary band',  fg: 'neutral0',   bg: 'primary',     min: 3.0, kind: 'focus' },
  { name: 'focus halo vs CTA',           fg: 'neutral0',   bg: 'accent',      min: 3.0, kind: 'focus' },
  { name: 'focus halo vs accent deep',   fg: 'neutral0',   bg: 'accentDeep',  min: 3.0, kind: 'focus' },
...
// ---------------------------------------------------------------- hard constraints
export function gate(cand) {
  const t = cand.tokens;
  const results = PAIRS_IN_USE.map((p) => {
    const ratio = contrast(t[p.fg], t[p.bg]);
    return { ...p, fgHex: t[p.fg], bgHex: t[p.bg], ratio, pass: ratio >= p.min };
  });
  ...
  const rotated = Object.entries(t).filter(([k]) => !(k in SEMANTIC) && !SKIP.has(k));
...
// ---------------------------------------------------------------- selection
export function selectPalette(masterSeed = 20260901, want = 5) {
...
if (isMain) {
  ...
  if (seedArg != null) {
    const cand = generate(seedArg);
    const g = gate(cand);
    if (argv.includes('--emit')) { console.log(emitTheme(cand)); process.exit(0); }
  ...
  if (argv.includes('--emit')) { console.log(emitTheme(sel.winner)); process.exit(0); }
  ...
  for (const r of sel.winner.gate.results) {
    console.log(`${r.name.padEnd(29)} | ${r.fgHex} | ${r.bgHex} | ${String(r.ratio).padStart(5)} | ${String(r.min).padStart(3)} | ${r.pass ? 'PASS' : 'FAIL'}`);
  }
}
