...
import {
  HARNESS, REFERENCE, ROUTE_MAP, BP_SET, browser, newPage, settle, writeJson, slug, summary,
} from './lib.mjs';
...
const PROBE_ASSETS = () => {
  ...
  // ---- icon FONTS rendered as glyphs (Divi ETmodules / FontAwesome / Gravity) ----
  const iconGlyphs = Array.from(document.querySelectorAll('*')).filter((el) => {
    const fam = cs(el).fontFamily || '';
    return /ETmodules|FontAwesome|dashicons|gform-icons/i.test(fam) && el.getBoundingClientRect().width > 0;
  }).map((el) => {
    const s = cs(el);
  ...
  const logoEl = document.querySelector('#logo, .et_pb_menu__logo img, #main-header img, header img');
  const logo = logoEl ? {
...
async function run(b, refPath, bp) {
  ...
  await settle(page);
  const data = await page.evaluate(PROBE_ASSETS);
  const file = path.join(HARNESS, 'assets', `${slug(refPath)}-${bp}.json`);
  await writeJson(file, { refPath, bp, ...data, network: net });
  await ctx.close();
  return {
    pass: `assets ${refPath} @${bp}`, imgs: data.imgs.length, bg: data.bg.length,
...
async function main() {
  ...
  const jobs = [];
  for (const p of Object.keys(ROUTE_MAP)) for (const bp of BP_SET) jobs.push({ p, bp });
  for (let i = 0; i < jobs.length; i += 2) {
    const r = await Promise.all(jobs.slice(i, i + 2).map((j) =>
      run(b, j.p, j.bp).catch((e) => ({ pass: `assets ${j.p} @${j.bp}`, error: e.message.slice(0, 110) }))));
    r.forEach(summary);
  }
  await b.close();
